...
| File | Description |
|------|-------------|
| `notebooks/r1-reaching-experiment.ipynb` | 実験の全手順を Jupyter notebook で再現（セットアップ → 学習 → 評価 → 比較可視化） |

notebook はパスを環境変数（`ISAACSIM_PATH`, `ISAACLAB_PATH`, `SO_ARM101_PATH`）で設定できるので、自分の環境に合わせて変更してください。Isaac Sim の Jupyter カーネル（`isaacsim.code_editor.jupyter`）での実行を想定しています。
